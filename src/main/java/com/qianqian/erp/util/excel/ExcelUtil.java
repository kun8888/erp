package com.qianqian.erp.util.excel;

import cn.hutool.core.date.DateUtil;
import cn.hutool.core.map.MapUtil;
import com.qianqian.erp.vo.ExcelSheetVo;
import com.qianqian.erp.vo.ExcelVo;
    ...
    public static List<Map<String, String>> analysisExcel(File file) throws Exception {
        ...
        for (int i = firstRowIndex; i <= lastRowIndex; i++) {
            ...
            if (null != row) {
                ...
                // 最后一列单价不要
                for (int cIndex = firstCellIndex; cIndex < lastCellIndex - 1; cIndex++) {   //遍历列
                    Cell cell = row.getCell(cIndex);
                    map.put(String.valueOf(cIndex), null == cell ? "" : cell.toString());
                }
                list.add(map);
            }
        }
